...
class Comprador_Model extends CI_Model {
	...
	public function getSenha($condicao) {		
		$this->db->select('senhacomprador, nomecomprador, sobrenomecomprador, emailcomprador, codcomprador');
		$this->db->where($condicao);
		$this->db->from('comprador');
	...
	public function get($condicao = array(), $primeiraLinha = FALSE, $pagina = 0, $limite = LINHAS_PESQUISA_DASHBOARD) {
		$this->db->select('codcomprador, nomecomprador, enderecocomprador, cidadecomprador, ufcomprador, cepcomprador, emailcomprador, newslettercomprador');
		$this->db->select('telefonecomprador, cpfcomprador, sexocomprador, senhacomprador, celularcomprador, sobrenomecomprador, datanascimentocomprador');
		$this->db->where($condicao);
		$this->db->from('comprador');
