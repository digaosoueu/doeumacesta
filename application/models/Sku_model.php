...
class Sku_Model extends CI_Model {
	...
	public function getPorSKU($codsku) {
		...
		$query = $this->db->query("SELECT s.codsku, s.referencia, s.quantidade, s.codproduto, p.nomeproduto, p.valorproduto, p.valorpromocional, p.peso, p.altura, p.largura, p.comprimento, pf.produtofoto FROM produto p inner join sku s on p.codproduto = s.codproduto inner join produtofoto pf on p.codproduto = pf.codproduto where s.codsku = " . $codsku);
