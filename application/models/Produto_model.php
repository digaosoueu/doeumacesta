...
class Produto_Model extends CI_Model {
	...
	public function get($condicao = array(), $primeiraLinha = FALSE, $pagina = 0, $limite = LINHAS_PESQUISA_DASHBOARD, $ordenacao = FALSE, $tipoOrdem = "ASC") {
		...
		$this->db->select ( 'peso, altura, largura, comprimento, codfabricante' );
		$this->db->where ( $condicao );
		$this->db->from ( 'produto' );
	...
	public function getBusca($condicao = array(), $primeiraLinha = FALSE, $pagina = 0, $limite = LINHAS_PESQUISA_DASHBOARD, $ordenacao = FALSE, $tipoOrdem = "ASC") {
		$this->db->select ( 'codproduto, nomeproduto, resumoproduto, fichaproduto, valorproduto, valorpromocional, codtipoatributo, urlseo, codfabricante' );
		foreach ( $condicao as $c ) {
	...
	public function listaFabricates($condicao = array(), $primeiraLinha = FALSE){
		
		$this->db->select ( 'codfabricante, nomefabricante' );
		$this->db->where ( $condicao );
