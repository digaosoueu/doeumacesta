...
class FaixaCepFreteGratis extends CI_Controller {
	...
	public function ver($codformaentrega) {
		...
		if ($totalPaginas > $pagina) {
			$data['HABPROX']	= null;
			$data['URLPROXIMO']	= site_url('painel/faixacepfretegratis?p='.($pagina+1));
		} else {
		...
		if ($pagina <= 1) {
			...
			$data['URLANTERIOR']= site_url('painel/faixacepfretegratis?p='.($pagina-1));
		}
		...
		while ($indicePg <= $totalPaginas) {
			$data['BLC_PAGINAS'][] = array(
				...
				"INDICE"	=> $indicePg,
				"URLLINK"	=> site_url('painel/faixacepfretegratis?p='.$indicePg)
			);
