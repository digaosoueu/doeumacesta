...
class FormaPagamento extends CI_Controller {
	...
	public function index() {
		...
		if ($res) {
			foreach($res as $r) {
				$data['BLC_DADOS'][] = array(
					...
					"MAXIMOPARCELA"=> ($r->maximoparcelasformapagamento==1)?'� Vista':$r->maximoparcelasformapagamento.' vezes',
				    "HABILITADO"   => ($r->habilitaformapagamento=='S')?'Sim':'N�o', 
		...
		if ($totalPaginas > $pagina) {
			$data['HABPROX']	= null;
			$data['URLPROXIMO']	= site_url('painel/formapagamento?p='.($pagina+1));
		} else {
		...
		if ($pagina <= 1) {
			...
			$data['URLANTERIOR']= site_url('painel/formapagamento?p='.($pagina-1));
		}
		...
		while ($indicePg <= $totalPaginas) {
			$data['BLC_PAGINAS'][] = array(
				...
				"INDICE"	=> $indicePg,
				"URLLINK"	=> site_url('painel/formapagamento?p='.$indicePg)
			);
	...
	public function adicionar() {
		...
		$data['sel_tipoformapagamento3']      = null;
		$data['sel_tipoformapagamento4']      = null;
	...
	public function editar($id) {
		...
		if ($res) {
			...
			show_error('Nao encontrados.', 500, 'Ops, erro encontrado.');
		}
	...
	public function excluir($id) {
		...
		if ($res) {
			...
			$this->session->set_flashdata('erro', 'Forma de pagamento n�o pode ser removido.');
		}
