...
class Produto extends CI_Controller {
	...
	public function index() {
		...
		if ($totalPaginas > $pagina) {
			$data['HABPROX']	= null;
			$data['URLPROXIMO']	= site_url('painel/produto?p='.($pagina+1));
		} else {
		...
		if ($pagina <= 1) {
			...
			$data['URLANTERIOR']= site_url('painel/produto?p='.$paginaVoltar);
		}
		...
		while ($indicePg <= $totalPaginas) {
			$data['BLC_PAGINAS'][] = array(
					...
					"INDICE"	=> $indicePg,
					"URLLINK"	=> site_url('painel/produto?p='.$indicePg)
			);
	...
	public function adicionar() {
		...
		$data['BLC_FABRICANTE']	= array();
		$data['BLC_DEPARTAMENTOPAI']= array();
		$data['des_tipoatributo']	= null;
		$data['des_fabricante']		= null;
		$data['readonly']	        = null;
		
		$fabricante	= $this->ProdutoM->listaFabricates(array(), FALSE);
		foreach($fabricante as $f){
			
			$data['BLC_FABRICANTE'][] = array(
					"CODFABRICANTE"			=> $f->codfabricante,
					"FABRICANTE"			=> $f->nomefabricante,
					"sel_codfabricante"		=> null
			);
		}
		...
		if ($depPai) {
			foreach($depPai as $dp) {
				...
				//DEFINE OS ELEMENTOS DO FILHO - IN�&#65533;CIO
				$aFilhos = array();
	...
	public function salvar() {
		...
		if (!$valorproduto) {
			...
			if ($valorpromocional) {
				if ($valorpromocional >= $valorproduto) {
					$erros		= TRUE;
					$mensagem	.= "Valor promocional nao pode ser maior que o valor de venda.\n";
				}
			}
		}
		...
		if (!$erros) {
			...
			if ($codproduto) {
				...
				$this->session->set_flashdata('erro', 'Ocorreu um erro ao realizar a operaca.');
			...
			if ($codproduto) {
				...
				redirect('painel/produto/adicionar/?erro=' .nl2br($mensagem));
			}
		}
	...
	public function editar($id) {
		...
		if ($res) {
			...
			show_error('Nao foram encontrados dados.', 500, 'Ops, erro encontrado.');
		}
		...
		foreach($fabricante as $f){
			...
			$data['BLC_FABRICANTE'][] = array(
					"CODFABRICANTE"			=> $f->codfabricante,
					"FABRICANTE"			=> $f->nomefabricante,
					"sel_codfabricante"		=> ($f->codfabricante == $res->codfabricante)?'selected="selected"':null);
		}
	...
	public function excluir($id) {
		...
		if ($res) {
			...
			$this->session->set_flashdata('erro', 'Produto nao pode ser removido.');
		}
	...
	public function salvafoto() {
		...
		//CRIA A MINIATURA DA P�&#65533;GINA INICIAL
	...
	private function redimensionaFoto($codprodutofoto, $extensao, $altura, $largura) {
		...
		$configImagem['image_library']	= 'gd2'; //BIBLIOTECA RESPONS�&#65533;VEL PELO REDIMENSIONAMENTO
