...
class Vitrine extends CI_Controller {
	...
	public function index() {
		...
		if ($totalPaginas > $pagina) {
			$data['HABPROX']	= null;
			$data['URLPROXIMO']	= site_url('painel/vitrine?p='.($pagina+1));
		} else {
		...
		if ($pagina <= 1) {
			...
			$data['URLANTERIOR']= site_url('painel/vitrine?p='.$pagina-1);
		}
	...
	public function produtos($codvitrine) {
	    ...
	    if ($totalPaginas > $pagina) {
	        $data['HABPROX']	= null;
	        $data['URLPROXIMO']	= site_url('painel/vitrine/produtos/'.$codvitrine.'?p='.($pagina+1));
	    } else {
	        $data['HABPROX']	= 'disabled';
	        $data['URLPROXIMO']	= '#';
	    }
	    ...
	    if ($pagina <= 1) {
	        ...
	        $data['HABANTERIOR']= null;
	        $data['URLANTERIOR']= site_url('painel/vitrine/produtos/'.$codvitrine.'?p='.$pagina-1);
	    }
	    ...
	    while ($indicePg <= $totalPaginas) {
	        $data['BLC_PAGINAS'][] = array(
	                "LINK"		=> ($indicePg==$pagina)?'active':null,
	                "INDICE"	=> $indicePg,
	                "URLLINK"	=> site_url('painel/vitrine/produtos/'.$codvitrine.'?p='.$indicePg)
	        );
