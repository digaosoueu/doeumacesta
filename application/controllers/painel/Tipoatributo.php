...
class TipoAtributo extends CI_Controller {
	...
	public function index() {
		...
		if ($totalPaginas > $pagina) {
			$data['HABPROX']	= null;
			$data['URLPROXIMO']	= site_url('painel/tipoatributo?p='.($pagina+1));
		} else {
		...
		if ($pagina <= 1) {
			...
			$data['URLANTERIOR']= site_url('painel/tipoatributo?p='.($pagina-1));
		}
		...
		while ($indicePg <= $totalPaginas) {
			$data['BLC_PAGINAS'][] = array(
					...
					"INDICE"	=> $indicePg,
					"URLLINK"	=> site_url('painel/tipoatributo?p='.$indicePg)
			);
	...
	public function salvar() {
		...
		if (!$erros) {
			...
			if ($codtipoatributo) {
				...
				$this->session->set_flashdata('erro', 'Ocorreu um erro ao realizar a operacao.');
	...
	public function excluir($id) {
		...
		if ($res) {
			...
			$this->session->set_flashdata('erro', 'Tipo de atributo nao pode ser removido.');
		}
